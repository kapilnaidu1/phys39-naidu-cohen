...
| Heat exchanger connected to 12 V and operating? | Liquid loop: an ID-COOLING radiator with two fans, feeding a coolant block on the underside of the plate. *To record: fans turning AND pump circulating, both confirmed before the TEC is energised.* Fan rotation alone does not prove the pump is running. **Check this one carefully:** the 23 Sept labelled run cooled to -6.79 C and then warmed back up at +0.33 C/s with the command still at duty 251, which is what a TEC does when hot-side heat is not being carried away. [`part7_labeled_run.txt`](../../data/module_03/part7_labeled_run.txt) |
...
| 6, serial command interface | `tec_python_control` | yes | **verified 23 Sept at Serial Monitor**, valid command, malformed input, clamping and zero-duty release. [`part6_serial_command_verification.txt`](../../data/module_03/part6_serial_command_verification.txt). Also drove the full 70 C run under the GUI |
| 7, integrated manual-control test | Parts 5 and 6 together | n/a | **all five tests done 23 Sept, checked by the instructor.** [`part7_integrated_manual_control_test.txt`](../../data/module_03/part7_integrated_manual_control_test.txt). Test 2 scope captures were not retained, recorded as a gap. Repeated with the corrected sketch: [`part7_labeled_run.txt`](../../data/module_03/part7_labeled_run.txt), **labels truthful**, 60.9 C swing, and the cooling leg reverses at -6.79 C under full command. Earlier wider run: Full drive, **-16.0 to +54.1 C, a 70.1 C swing**, 37.5 C below ambient and 32.6 C above. [`part7_full_drive_70C_swing.txt`](../../data/module_03/part7_full_drive_70C_swing.txt) and its CSV. Earlier partial-drive attempt kept at [`part7_heat_cool_cycle_gui.txt`](../../data/module_03/part7_heat_cool_cycle_gui.txt). Direction labels inverted in both, section 6 |
| 8, cleanup and C3 | n/a | repository layout done, this note started | pending the Part 1 rows above |
